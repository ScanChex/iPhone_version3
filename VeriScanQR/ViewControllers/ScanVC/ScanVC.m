...
- (void)viewDidLoad
{
  ...
  self.shareURL=[NSURL URLWithString:@""];
  self.status=@"";
...
-(void)viewDidDisappear:(BOOL)animated{
  [[NSNotificationCenter defaultCenter] removeObserver:self name:PDF_FILE_SHARE object:nil];
   }
...
-(void)selectedFileWithPath:(DocumentDTO *)document{
  
  self.document_id=document.document_id;
  version=[document.version intValue];
  
  NSString *filePath = [document.link stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
-(void)askDocumentStatus{
  
  UIAlertView * tempAlert = [[[UIAlertView alloc]initWithTitle:@"New Message" message:@"Is this document completed" delegate:self cancelButtonTitle:nil otherButtonTitles:@"YES",@"NO",  nil] autorelease];
  [tempAlert setTag:-1022];
  [tempAlert show];
...
-(void)uploadPDFWithStatusFilePath:(NSURL *)filePath
{
  ...
  UserDTO*user=[[VSSharedManager sharedManager] currentUser];
  [SVProgressHUD showWithStatus:@"Uploading..." maskType:SVProgressHUDMaskTypeBlack];
  [[WebServiceManager sharedManager] uploadPdfWithStatus:[NSString stringWithFormat:@"%d",user.masterKey] historyID:[[VSSharedManager sharedManager] historyID] contentType:@"application/pdf" pdfData:fileData assetId:ticketAsset.assetID status:self.status document_id:self.document_id   version:[NSString stringWithFormat:@"%d",version]
                                               ticket_id:selectedTicketInfo.tblTicketID
                                                username:[[NSUserDefaults standardUserDefaults] objectForKey:@"userID"] progressBar:self.myProgressIndicator withCompletionHandler:^(id data,BOOL error){
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
  ...
  if ([alertView tag] == -1022) {
    ...
    [self uploadPDFWithStatusFilePath:self.shareURL];
